...
class AfasUpdateConnector extends AfasConnector implements iAfasElement {
  ...
  /**
   * Prefills element following the definition.
   *
   * @param AfasElement $p_oElement
   * @param string $p_sDefinition
   *
   * @access public
   */
  public function applyDefinition(AfasElement $p_oElement, $p_sDefinition) {
    // Prefill if definition is available
    $update_connectors = afas_get_update_connectors();
    if (isset($update_connectors[$p_sDefinition])) {
      // Load definitions
      $definition = $update_connectors[$p_sDefinition]->getDefinition();
      $this->_applyDefinitionHelper($p_oElement, $definition[$p_sDefinition]['Element']);
    }
  }

  /**
   * Helper function for addElementWithDefinition().
   *
   * @param AfasElement $p_oElement
   * @param array $p_aDefElement
   *
   * @access private
   * @return array
   */
  private function _applyDefinitionHelper(AfasElement $p_oElement, $p_aDefElement) {
    // Prefill.
    foreach (element_children($p_aDefElement['Fields']) as $sFieldName) {
      $aFieldinfo = $p_aDefElement['Fields'][$sFieldName];
      $mValue = '';
      if ($aFieldinfo['default_value'] !== '') {
        $mValue = $aFieldinfo['default_value'];
      }
      else {
      ...
      $p_oElement->setField($sFieldName, $mValue);
    }

    // Prefill underlying objects.
    if (isset($p_aDefElement['Objects'])) {
      foreach (element_children($p_aDefElement['Objects']) as $sObjectName) {
        $oChild = $p_oElement->addChild($sObjectName);
        $aObjectinfo = $p_aDefElement['Objects'][$sObjectName];
        $this->_applyDefinitionHelper($oChild, $aObjectinfo['Element']);
      }
    }
  }
}
