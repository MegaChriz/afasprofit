...
class UpdateConnectorTest extends ConnectorTestBase {
  ...
  public function testConstructWithoutEntityContainer() {
    ...
    $this->assertInstanceOf(EntityContainerInterface::class, PHPUnit_Framework_Assert::readAttribute($connector, 'entityContainer'));
  }
  ...
  public function testConstructWithEntityContainer() {
    ...
    $entity_container = $this->getMock(EntityContainerInterface::class);
    $connector = new UpdateConnector($this->client, $this->server, 'FbSales', $entity_container);
    $this->assertEquals($entity_container, PHPUnit_Framework_Assert::readAttribute($connector, 'entityContainer'));

    // New entity container.
    $entity_container2 = new EntityContainer('FbSales');
    $connector->setEntityContainer($entity_container2);
    $this->assertNotEquals($entity_container, PHPUnit_Framework_Assert::readAttribute($connector, 'entityContainer'));
    $this->assertEquals($entity_container2, PHPUnit_Framework_Assert::readAttribute($connector, 'entityContainer'));
  }
